...
namespace imat3111
{
	...
	void SceneDeferred::initScene(QuatCamera camera)
	{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	void SceneDeferred::setLightParams(QuatCamera camera)
	{
		...
		prog.setUniform("Light.linear", 0.007f);
		prog.setUniform("Light.quadratic", 0.0002f);
	}
	...
	void SceneDeferred::render(QuatCamera camera)
	{
		pass1(camera);
		pass2(camera);
	}

	void SceneDeferred::pass1(QuatCamera camera)
	{
		gl::BindFramebuffer(gl::FRAMEBUFFER, deferredFBO);
		...
		prog.setUniform("Material.spec", 10.0f);
		...
		prog.setUniform("Material.Ks", 1.0f, 1.0f, 1.0f);
		prog.setUniform("Material.spec", 50.0f);
		model = mat4(1.0f);
		setMatrices(camera);
		plane->render();
	...
	void SceneDeferred::pass2(QuatCamera camera)
	{
	...
	void SceneDeferred::setMatrices(QuatCamera camera)
	{
	...
	void SceneDeferred::resize(QuatCamera camera, int w, int h)
	{
		gl::Viewport(0, 0, w, h);
		width = w;
		height = h;
	...
	void SceneDeferred::compileAndLinkShader()
	{
		...
		try {
			...
			prog.compileShader("Shaders/deferred.vs");
			prog.compileShader("Shaders/deferred.fs");
			prog.link();
			prog.validate();
			prog.use();
		}
