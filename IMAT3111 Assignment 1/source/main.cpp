...
static void key_callback(GLFWwindow* window, int key, int cancode, int action, int mods)
{
	...
	if (key == 'R' && action == GLFW_RELEASE) //Reset the camera position
		camera.reset();
	if (key == 'F' && action == GLFW_RELEASE) //Toggle camera Yaw  rotation
		camera.toggleYaw();
	if (key == 'G' && action == GLFW_RELEASE) //Toggle camera Pitch Rotation
		camera.togglePitch();
	if (key == 'M' && action == GLFW_RELEASE) //Toggle skybox rendering
		scene->toggleCubeMap();
	if (key == 'J' && action == GLFW_RELEASE)
		scene->toggleRefract();
}
...
void glfwErrorCallBack(int errorCode, const char * eString) {
	int t = 1;
}
